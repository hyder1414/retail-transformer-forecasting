...
def _build_sequences(df: pd.DataFrame, feature_columns: List[str], seq_len: int, horizon: int, ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    def create_seq(group: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        X_vals = group[feature_columns].values.astype(np.float32)
        y_vals = group["Weekly_Sales_Log"].values.astype(np.float32)
        holiday_vals = group["IsHoliday"].values.astype(np.int8)

        seqs_x, seqs_y, seqs_h = [], [], []
        ...
        for start in range(max_start):
            ...
            seqs_h.append(holiday_vals[end:out_end])
        if not seqs_x:
            return (
                np.empty((0, seq_len, len(feature_columns)), dtype=np.float32),
                np.empty((0, horizon), dtype=np.float32),
                np.empty((0, horizon), dtype=np.int8),
            )
        return np.stack(seqs_x), np.stack(seqs_y), np.stack(seqs_h)
    X_list, y_list, yhol_list = [], [], []
    for (_, _), g in df.groupby(["Store", "Dept"], sort=False):
        if len(g) >= seq_len + horizon:
            xs, ys, ys_h = create_seq(g)
            if xs.shape[0] > 0:
                X_list.append(xs)
                y_list.append(ys)
                yhol_list.append(ys_h)

    if len(X_list) == 0:
        raise ValueError("Couldn't create seq, check  input parameters")
    X = np.concatenate(X_list, axis=0)
    y = np.concatenate(y_list, axis=0)
    y_holiday = np.concatenate(yhol_list, axis=0)
    return X, y, y_holiday
...
def prepare_walmart_data(
    ...
    yhol_path = data_dir / f"yhol_seq{seq_len}_h{horizon}.npy"
    ...
    if (
        ...
        return X, y, y_holiday, scaler, feature_columns
    ...
    X, y, y_holiday = _build_sequences(df, feature_columns, seq_len=seq_len, horizon=horizon)

    # Save Cache again
    np.save(X_path, X)
    np.save(y_path, y)
    np.save(yhol_path, y_holiday)
...
if __name__ == "__main__":
    X, y, y_holiday, scaler, feature_cols = prepare_walmart_data(
        seq_len=16,
        horizon=4,
        data_dir="cache",
