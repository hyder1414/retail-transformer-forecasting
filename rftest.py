...
from dataload import prepare_walmart_data
...
def main():
    ...
    mse1 = mean_squared_error(y_val_sales, y_pred_sales)
    rmse1 = np.sqrt(mse1)
    r21 = r2_score(y_val_sales, y_pred_sales)
    print(f"mse: {mse1}$,  rmse:{rmse1}$, r2:{r21}")
    print("\nSAMPLE PREDICTIONS")
    #wmae score 
    # Weighted MAE (WMAE): weight = 5 for holiday weeks, else 1
    weights = np.where(yhol_val == 1, 5.0, 1.0)
    wmae = np.sum(weights * np.abs(y_val_sales - y_pred_sales)) / np.sum(weights)
